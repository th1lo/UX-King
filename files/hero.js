var images = {
  ...
  text: ["UX & Webdesign Agentur aus Berlin","Make Your UX Great Again","We Can Fix Your Product Retention", "Better UX For Harambe", "We Help You Create Innovative Products","Never Stop Improving"]
};

window.setInterval(function(){
 var random = Math.floor(Math.random() * images.background.length);
 $('#hero').css({'background-image': 'url(' + images.background[random] + ')'});
 $('.memo').html(images.text[random]+ '<span class="dot">.</span>').textillate({ in: { effect: 'fadeIn' } });
}, 5000);


var random = Math.floor(Math.random() * images.background.length);
$('#hero').css({'background-image': 'url(' + images.background[random] + ')'});
$('.memo').html(images.text[random]+ '<span class="dot">.</span>');
